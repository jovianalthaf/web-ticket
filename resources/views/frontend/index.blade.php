    ...
    <section id="eventSection" class="container relative max-w-screen-xl py-10">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[30px]">
            ...
            @foreach ($events as $event)
                <x-frontend.card-event :cover="$event->thumbnail" :title="$event->name" :category="$event->category->name" :date="$event->start_time->format('d M Y')"
                    :price="$event->start_from" :isPopular="$event->is_popular" :description="$event->headline" :route="route('detail', $event->slug)" />
            @endforeach
